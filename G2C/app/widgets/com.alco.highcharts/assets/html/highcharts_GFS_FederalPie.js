...
(function() {
	...
	function Ba(a, b, c, d) {
		var e = K.numberFormat,
		    ...
		    i = c ===
		void 0 ? f.decimalPoint : c,
		    f = d ===
		void 0 ? f.thousandsSep : d,
		    j = g < 0 ? "-" : "",
			...
			init : function(b) {
				...
				a.fn.highcharts = function() {
					...
					if (this[0]) {
						...
						c = b[0];
						if (c !== u)
							c.chart = c.chart || {}, c.chart.renderTo = this[0], new K[a](c, b[1]),
							g = this;
	...
	S.prototype = {
		...
		align : function(a, b, c) {
			...
			if (a) {
				if (this.alignOptions = a, this.alignByTranslate =
					b, !c || Ga(c))
					this.alignTo = d = c || "renderer", la(f, this), f.push(this),
					c = null
			} else
				a = this.alignOptions,
		...
		shadow : function(a, b, c) {
			...
			if (a) {
				...
				for ( e = 1; e <= i; e++) {
					...
					if (c)
						F(f, "height", t(F(f, "height") - h, 0)), f.cutHeight =
						h;
		...
		textSetter : function(a) {
			if (a !== this.textStr)
				...
				a, this.added && this.renderer.buildText(this)
		},
	...
	ta.prototype = {
		...
		buildText : function(a) {
			...
			i = /<.*href="(http[^"]+)".*>/, l && !a.added && this.box.appendChild(b),
			e = f ? e.replace(/<(b|strong)>/g, '<span style="font-weight:bold">').replace(/<(i|em)>/g, '<span style="font-style:italic">').replace(/<a/g, "<span").replace(/<\/(b|strong|i|em|a)>/g, "</span>").split(/<br.*?>/g) : [e], e[e.length - 1] === "" && e.pop(), q(e, function(e, f) {
				var g,
				    n = 0,
						...
						if (e !== " ") {
							...
							if (l)
								for (var e = e.replace(/([^\^])-/g, "$1- ").split(" "),
								    m = g.length > 1 || e.length > 1 && k.whiteSpace !== "nowrap",
								    q,
								    D,
								    s = k.HcHeight,
								    t = [],
								    u = Y(o),
								    Lb = 1; m && (e.length || t.length); )
									delete a.bBox,
									q = a.getBBox(),
									D = q.width, !ba && c.forExport && ( D = c.measureSpanWidth(o.firstChild.data, a.styles)),
									q = D > l, !q || e.length === 1 ? ( e = t,
									t = [], e.length && (Lb++, s && Lb * u > s ? ( e = ["..."], a.attr("title", a.textStr)) : ( o = x.createElementNS(xa, "tspan"), F(o, {
										dy : u,
										x : j
		...
		label : function(a, b, c, d, e, f, g, h, i) {
			function j() {
				...
				if (y) {
					if (!p)
						a = v(-J * C),
						b = h ? -R : 0, m.box = p = d ? n.symbol(d, a, b, m.width, m.height, z) : n.rect(a, b, m.width, m.height, 0, z[Qb]), p.attr("fill", P).add(m);
					p.isImg || p.attr(r({
						width : v(m.width),
						height : v(m.height)
					}, z));
				...
				if (i) {
					...
					if (c === "fill")
						if (i === "gradient")
							...
							m = n.x2 || n[2] || 0,
							n = n.y2 || n[3] || 0,
							C = 'angle="' + (90 - V.atan((n - a) / (m - c)) * 180 / oa) + '"', u();
						else {
							var j = n.r,
							    r = j * 2,
							    v = j * 2,
					...
					f = ya(a),
					h = ["<", c, ' opacity="', f.get("a"), '"/>'], $(this.prepVML(h), null, null, b),
					j = f.get("rgb");
				else {
	...
	Ta.prototype = {
		...
		render : function(a, b, c) {
			...
			if (n && J)
				...
				h = this.getMarkPath(x, w, J, n * z, g, f), m ? m.animate({
					d : h,
					opacity : c
	...
	na.prototype = {
		...
		translate : function(a, b, c, d, e, f) {
			...
			b ? ( a = a * g + h, a -= j,
			a = a / i + d, e && ( a = this.lin2val(a))) : (e && ( a = this.val2lin(a)), f === "between" && ( f = 0.5),
			a = g * (a - d) * i + h + g * j + (ja(f) ? i * f * this.pointRange : 0));
			return a
		},
		...
		getPlotLinePath : function(a, b, c, d, e) {
			...
			if (isNaN(e))
				...
				if ( i = h,
					j = k - this.bottom, a < g || a > g + this.width)
					n = !0
			} else if ( a = g,
				c = l - this.right, i < h || i > h + this.height)
				n = !0;
		...
		setAxisTranslation : function(a) {
			...
			if (b.isXAxis || i || d)
				...
				h = b.ordinalSlope && e ? b.ordinalSlope / e : 1, b.minPointOffset = f *= h, b.pointRangePadding = g *= h, b.pointRange = L(d, c), b.closestPointRange =
				e;
			if (a)
				b.oldTransA = j;
		...
		setTickPositions : function(a) {
			...
			c = b.linkedParent.getExtremes(), b.min = p(c.min, c.dataMin), b.max = p(c.max, c.dataMax), d.type !== b.linkedParent.options.type && ha(11, 1)) : (b.min = p(b.userMin, d.min, b.dataMin), b.max = p(b.userMax, d.max, b.dataMax));
			if (g)
			...
			if (!a)
				...
				a = h ? b.getTimeTicks(b.normalizeTimeTickInterval(b.tickInterval, d.units), b.min, b.max, d.startOfWeek, b.ordinalPositions, b.closestPointRange, !0) : g ? b.getLogTickPositions(b.tickInterval, b.min, b.max) : b.getLinearTickPositions(b.tickInterval, b.min, b.max), D && a.splice(1, a.length - 2), b.tickPositions =
				a;
			if (!j)
				d = a[0],
				g = a[a.length - 1],
				h = b.minPointOffset || 0, e ? b.min = d : b.min - h > d && a.shift(), f ? b.max = g : b.max + h < g && a.pop(), a.length === 0 && s(d) && a.push((g + d) / 2), a.length === 1 && ( e = Q(b.max) > 1E13 ? 1 : 0.001, b.min -= e, b.max += e)
		},
		...
		getOffset : function() {
			...
			if (j || a.isLinked) {
				...
				if (a.staggerLines)
					D *= a.staggerLines, a.labelOffset =
					D
			} else
	...
	na.prototype.getTimeTicks = function(a, b, c, d) {
		...
		if (s(b)) {
			...
			j >= A.month && (i[Eb](j >= A.year ? 0 : k * U(i[fb]() / k)),
			h = i[gb]());
			...
			for (var d = i.getTime(),
			    l = i[fb](),
			    n = i[Xa](),
			    m = (A.day + ( g ? Sa : i.getTimezoneOffset() * 6E4)) % A.day; d < c; )
	...
	na.prototype.normalizeTimeTickInterval = function(a, b) {
		...
		for ( g = 0; g < c.length; g++)
			if ( d = c[g],
				e = A[d[0]],
				f = d[1], c[g + 1] && a <= (e * f[f.length - 1] + A[c[g+1][0]]) / 2)
				break;
	...
	na.prototype.getLogTickPositions = function(a, b, c, d) {
		...
		if (a >= 0.5)
			...
			for (var f = U(b),
				...
				i = e.length;
				for ( h = 0; h < i && !l; h++)
					j = za(ka(f) * e[h]), j > b && (!d || k <= c) && k !== u && g.push(k), k > c && ( l = !0),
					k =
					j
			}
		else if ( b = ka(b),
			...
			a = nb(a, null, mb(a)),
			g = Va(this.getLinearTickPositions(a, b, c), za), !d)
	...
	Wa.prototype = {
		...
		normalize : function(a, b) {
			...
			d.pageX === u ? ( c = t(a.x, a.clientX - b.left),
	...
	lb.prototype = {
		init : function(a, b) {
			...
			this.options = b;
			if (b.enabled)
				c.itemStyle = d, c.itemHiddenStyle = w(d, b.itemHiddenStyle), c.itemMarginTop =
				f, c.padding =
				e, c.initialItemX =
				e, c.initialItemY = e - 5, c.maxItemWidth = 0, c.chart =
				a, c.itemHeight = 0, c.lastLineHeight = 0, c.symbolWidth = p(b.symbolWidth, 16), c.pages = [], c.render(), N(c.chart, "endResize", function() {
		...
		colorizeItem : function(a, b) {
			...
			if (f) {
				if (g && f.isMarker)
					for (j in i.stroke = h,
					g = a.convertAttribs(g), g) d = g[j], d !== u && (i[j] = d);
				f.attr(i)
			}
		},
		...
		renderItem : function(a) {
			...
			e ? this.itemX += f : (this.itemY += o + g + m, this.lastLineHeight =
			g);
	...
	O.prototype = {
		...
		generatePoints : function() {
			...
			if (!b && !j)
				...
				b = this.data = b;
			for ( n = 0; n < g; n++)
				i = h + n, j ? l[n] = (new f).init(this, [d[n]].concat(ra(e[n]))) : (b[i] ? k = b[i] : a[i] !== u && (b[i] = k = (new f).init(this, a[i], d[n])), l[n] =
				k), l[n].index =
				i;
			if (b && (g !== ( c = b.length) || j))
				for ( n = 0; n < c; n++)
					if (n === h && !j && (n += g), b[n])
						b[n].destroyElements(), b[n].plotX =
						u;
			this.data = b;
			this.points = l
		},
		...
		translate : function() {
			...
			for (var a = this.options,
				...
				if (b && this.visible && q && q[n])
					...
					o = m[0],
					m = m[1], o === 0 && ( o = p(k, e.min)), e.isLog && o <= 0 && ( o = null), l.total = l.stackTotal = q.total, l.percentage = q.total && l.y / q.total * 100, l.stackY =
					m, q.setOffset(this.pointXOffset || 0, this.barW || 0);
		...
		animate : function(a) {
			...
			a ? ( a = b[g],
			d = b[g + "m"], a || (b[g] = a = c.clipRect(r(e, {
				width : 0
			...
			if (l.enabled !== !1 || this._hasPointMarkers)
				for ( f = b.length; f--; )
					if ( g = b[f],
						...
						q = c.isInsidePlot(v(d), e, c.inverted), a && e !== u && !isNaN(e) && g.y !== null)
	...
	Gb.prototype = {
		...
		setOffset : function(a, b) {
			...
			if ( e = this.label)
				...
				f = e.alignAttr, e[this.options.crop===!1||d.isInsidePlot(f.x,f.y)?"show":"hide"](!0)
		}
	};
	...
	ia = ma(O, {
		...
		getPointSpline : function(a, b, c) {
			...
			if (f && g) {
				...
				k += l;
				i > a && i > e ? ( i = t(a, e),
				k = 2 * e - i) : i < a && i < e && ( i = L(a, e),
				k = 2 * e - i);
				k > g && k > e ? ( k = t(g, e),
				i = 2 * e - k) : k < g && k < e && ( k = L(g, e),
				i = 2 * e - k);
				b.rightContX = j;
				b.rightContY = k
			}
	...
	ia = ma(O, {
		...
		getColumnMetrics : function() {
			...
			b.grouping === !1 ? i = 1 : q(a.chart.series, function(b) {
				...
				if (b.type === a.type && b.visible && d.len === e.len && d.pos === e.pos)
					c.stacking ? ( f = b.stackKey, g[f] === u && (g[f] = i++),
		...
		animate : function(a) {
			...
			if (ba)
				a ? (e.scaleY = 0.001,
				a = L(b.pos + b.len, t(b.pos, b.toPixels(c.threshold))), d ? e.translateX = a - b.len : e.translateY = a, this.group.attr(e)) : (e.scaleY = 1, e[ d ? "translateX" : "translateY"] = b.pos, this.group.animate(e, this.options.animation), this.animate = null)
		},
		remove : function() {
			var a = this,
			    b = a.chart;
	...
	O.prototype.alignDataLabel = function(a, b, c, d, e) {
		...
		if ( a = this.visible && (a.series.forceDL || f.isInsidePlot(h, v(i), g) || d && f.isInsidePlot(h, g ? d.x + 1 : d.y + d.height - 1, g)))
			...
			g = b.alignAttr, p(c.overflow, "justify") === "justify" ? this.justifyDataLabel(b, c, g, j, d, e) : p(c.crop, !0) && ( a = f.isInsidePlot(g.x, g.y) && f.isInsidePlot(g.x + j.width, g.y + j.height)));
		if (!a)
			b.attr({
				y : -999
			}), b.placed = !1
	...
	if (H.pie)
					...
					if (K) {
						...
						for ( z = 0; z < K; z++) {
							...
							if (n > 0) {
								if ( w = M.pop(),
									E = w.i,
									B = w.y, c > B && G[E + 1] !== null || c < B && G[E - 1] !== null)
				...
				if (Ca(R) === 0 || this.verifyDataLabelOverflow(R))
					this.placeDataLabels(), s && g && q(this.points, function(b) {
						...
						if (( u = b.dataLabel) && u._pos)
							...
							y = u.connX,
							B = u.connY,
							k = l ? ["M", y + (x[6] === "left" ? 5 : -5), B, "C", y, B, 2 * x[2] - x[4], 2 * x[3] - x[5], x[2], x[3], "L", x[4], x[5]] : ["M", y + (x[6] === "left" ? 5 : -5), B, "L", x[2], x[3], "L", x[4], x[5]], j ? (j.animate({
								d : k
		...
		}, H.pie.prototype.alignDataLabel =
		sa, H.pie.prototype.verifyDataLabelOverflow = function(a) {
			var b = this.center,
			    c = this.options,
			    d = c.center,
			    e = c = c.minSize || 80,
		...
		setState : function(a, b) {
			...
			if (!(a === this.state && !b || this.selected && a !== "select" || f[a] && f[a].enabled === !1 || a && (j || h && i.enabled === !1) || a && l.states && l.states[a] && l.states[a].enabled === !1)) {
				if (this.graphic)
					...
					if (a && i)
						if ( g = i.radius,
							l = l.symbol || e.symbol, k && k.currentSymbol !== l && ( k = k.destroy()), k)
							k[b?"animate":"attr"]({
								x : c - g,
								y : d - g
							});
						else if (l)
							e.stateMarkerGraphic = k = n.renderer.symbol(l, c - g, d - g, 2 * g, 2 * g).attr(o).add(e.markerGroup), k.currentSymbol =
							l;
					if (k)
