...
function getStep2Data() {
	...
	httpManager.getSecondStepRegistration(mappingId, function(e) {
		...
		var delay = setTimeout(function(e) {
			...
			httpManager.getSecurityAnswerForMSupplier($.txtEmailId.value.trim(), function(e) {
...
function getStep3Data() {
	...
	httpManager.getThirdStepRegistration(registerId, "", function(e) {
...
function callWSforFirstStep() {
	...
	var obj = {
		...
		contactRequestId : contactId,//doc.contactReqId
	};
...
function submitForm() {
	...
	if (currentViewIndex == 1) {
		...
		if (paymentStatus == "APPROVED" || paymentStatus == "PENDING_APPROVAL" || supplierStatus == "R") {
...
$.contactTableViewItems.addEventListener("itemclick", function(e) {
	...
	//	Ti.API.info("tableview  "+JSON.stringify(item.properties.obj));
...
function loadActivityItems(arrDoc) {
	...
	for (var i = 0; i < arrDoc.length; i++) {
		...
		Ti.API.info('arrDoc activity' + arrDoc[i]);
...
function changeLanguage() {
	...
	$.txtCompanyNameAr.value = doc.supplierNameAr;
	$.txtCompanyRegNo.value = $.txtCertificate2.value = doc.registrationNumber;
	$.txtCompanyNameEn.value = doc.supplierNameEn;
	//	$.txtFirstName.value = doc.firstName;
	$.txtEmailId.value = doc.emailId;
	$.txtContactName.value = doc.contactName;
	$.txtMobileNo.value = doc.mobileNo;
	$.txtTelNo.value = doc.telPhoneNo;
	contactId = doc.contactReqId;
...
function changeOrientation(e) {
	Ti.API.info('changeOrientation==' + JSON.stringify(e));
...
$.winResumeRegistration.addEventListener("close", function(e) {
