...
if (Alloy.Globals.isIOS7Plus) {
	...
	$.winFGFinalAccountsReports.extendEdges = [Ti.UI.EXTEND_EDGE_TOP];
	$.statusBar.height = 20;
}
...
function loadItems(arrDoc, isFirstLoad) {
	...
	for (var i = 0; i < arrDoc.length; i++) {
		...
		row.addEventListener("click", function(e) {
			...
			switch(index) {
			case 0:
				openbudgetExecutionByRevenue(0);
				break;
			case 1 :
...
function closeWindow() {
	Alloy.Globals.closeWindow($.winFGFinalAccountsReports);
}
...
$.viewBottomToolbar.getView().addEventListener('click', function(e) {
	if(e.source.buttonId == 'btnFontChange'){
		$.viewBottomToolbar.changeFont($.winFGFinalAccountsReports);
		...
		$.viewBottomToolbar.changeTheme($.winFGFinalAccountsReports);
	}
});
/**
...
var windowOpened = function(e){
	Alloy.Globals.arrWindows.push($.winFGFinalAccountsReports);
...
$.winFGFinalAccountsReports.addEventListener("focus", function(e) {
	isClicked = false;
	$.viewBottomToolbar.setDefaultTheme($.winFGFinalAccountsReports);
});
$.viewBottomToolbar.setDefaultTheme($.winFGFinalAccountsReports);
