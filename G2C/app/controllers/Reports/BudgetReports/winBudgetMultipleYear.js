...
var density;
var selectedChartStyle = "BUDGET MULTIPLE YEAR";
...
$.lblBudgetActivity.text = "Budget By : 2014 - 2016";
...
$.winBudgetMultipleYear.addEventListener("open", function(e) {
...
if (Alloy.Globals.isIOS7Plus) {
	//	$.winBudgetMultipleYear.statusBarStyle = Titanium.UI.iPhone.StatusBar.LIGHT_CONTENT;
	$.winBudgetMultipleYear.extendEdges = [Ti.UI.EXTEND_EDGE_TOP];
	$.statusBar.height = 20;
	$.budgetView.top = 75;
	//	$.sectorView.top = 65;
...
function showBudgetView() {
	...
	if (!isBudgetViewOpened) {
		...
		/*if (isNavChartHidden) {
		 //	selectedChartStyle  = "BUDGET MULTIPLE YEAR";
		 loadChartWithStyle("BUDGET MULTIPLE YEAR");
		 } else {
...
function closeWindow() {
	Alloy.Globals.closeWindow($.winBudgetMultipleYear);
}
...
function loadRows(arr, value) {
	...
	for (var i = 0; i < arr.length; i++) {
		...
		var payLoad = {
			...
			group21 : arr[i].year1 / 1000000,
			group22 : arr[i].year2 / 1000000,
			group31 : arr[i].year3 / 1000000,
...
function loadBudgetRows() {
	...
	for (var i = 0; i < arrBudget.length; i++) {
		...
		row.addEventListener("click", function(e) {
			...
			httpManager.BudgetReportsByYear(e.source.doc.group, function(e) {
...
function loadChartData(arr) {
	...
	for (var i = 0; i < arr.length; i++) {
		...
		year1.push(parseFloat(arr[i].year1));
		year2.push(parseFloat(arr[i].year2));
		year3.push(parseFloat(arr[i].year3));
...
function changeLanguage() {
	...
	$.lblCode.text = "Code";
	$.lblGroup21.text = "2014";
	$.lblGroup22.text = "2015";
	$.lblGroup31.text = "2016";
...
function loadChartWithStyle(style) {
	...
	/*if (style == "PIE") {
	 ...
	 } else if (style == "BUDGET MULTIPLE YEAR") {
	 var region = [{
	 name : 'Program 1',
	 y : 30,
	 sliced : false,
...
$.winBudgetMultipleYear.addEventListener("close", function(e) {
