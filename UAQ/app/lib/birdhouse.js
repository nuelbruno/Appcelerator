		...
		try {
			...
			function checkStatus() {
				...
				if (!doinOurThing) {
					...
					if (!loading && webURL.indexOf("oauth/authorize") <= -1) {
						webView.stopLoading();
						win.remove(webView);
						win.close();
			...
			webView.addEventListener('load', function(e) {
				...
				//https://api.twitter.com/oauth/authorize?oauth_token=gZaHmh8rsnL46fouaFoXlw71jIajkhLzHrjJKY94
				webURL = e.url.toString();
				if (loads == 2) {
					// something else was clicked
					Titanium.API.info('---11-----' + e.url + "  --------------------- " + e.url.indexOf("oauth/authorize"));

					// if (e.url != 'https://api.twitter.com/oauth/authorize') {
					if (webURL.indexOf("oauth/authorize") <= -1) {
		...
		try {
			...
			// user is authorized so execute API
				...
				XHR.onload = function(ee) {
					Ti.API.info('ONLOAD EVENT :' + JSON.stringify(ee));
					// execute the callback function
					if ( typeof (callback) == 'function') {
				...
				Ti.API.info('---FINAL URL------' + finalUrl);
	...
	*/
	...
	var sendTwitterImage = function(postParams, pSuccessCallback, pErrorCallback) {
		...
		if (!authorized && ( typeof (auth) == 'undefined' || auth === true)) {
			...
			XHR.onload = function() {
				Ti.API.info("--------Successfully image share---- " + XHR.responseText);
				if (pSuccessCallback) {
