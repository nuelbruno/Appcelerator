...
var OAuthAdapter = function(pConsumerSecret, pConsumerKey, pSignatureMethod) {
	...
	this.sendTwitterImage = function(options) {
		...
		client = Ti.Network.createHTTPClient({
			...
			onerror : function(e) {
				...
				Ti.API.info("2. Social.js: FAILED to send a request!");
				pErrorMessage && pErrorMessage(this.responseText);
			}
		});
	...
	this.closeWebView = function(cb) {
		...
		}), Ti.Android || (window.opacity = 1, window.transform = Ti.UI.create2DMatrix().scale()), view = Ti.UI.createView({
			top : 10,
			right : 10,
			bottom : 10,
			left : 10,
		...
		client = Ti.Network.createHTTPClient({
			...
			onerror : function() {
				Ti.API.info("3. Social.js: FAILED to send a request!");
				Ti.API.info('Error data '+this.responseText);
				pErrorMessage && pErrorMessage(this.responseText);
			}
		});
		...
		client = Ti.Network.createHTTPClient({
			...
			onerror : function() {
				Ti.API.info("4. Social.js: FAILED to send a request!");
		...
		Ti.API.info('3 '+options.url);
		Ti.API.info('4 '+JSON.stringify(options));
		...
		var parameterMap = OAuth.getParameterMap(message.parameters), client = Ti.Network.createHTTPClient({
			...
			onerror : function() {
				Ti.API.info("1. Social.js: FAILED to send a request!"), pErrorMessage && pErrorMessage(this.responseText);
			}
		});
	...
	return urls == null ? (alert("The Social.js module does not support " + site + " yet!"), null) : {
		...
		share : function(options) {
			this.authorize(function() {
				...
				Ti.API.info('adapter '+JSON.stringify(adapter));
			});
		},
