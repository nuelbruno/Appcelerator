...
(function() {
	PictureGallery.createWindow = function(dictionary) {
		...
		dictionary.isVideoGallery = ( typeof dictionary.isVideoGallery == 'undefined') ? false : dictionary.isVideoGallery;
	
	Ti.API.info("Is videogallery :"+ dictionary.isVideoGallery);
		// ------- Methods -------
		/**
		...
		var createThumbGallery = function() {
			...
			for (var i = 0, b = dictionary.images.length; i < b; i++) {
				...
				var playIcon = Ti.UI.createImageView({
					...
					image : Ti.App.ResourcePath + 'common/video_icon.png',
					//imageId : i,
				...
				if(dictionary.isVideoGallery) // Video gallery
				{
				    thumbImageBorder.add(playIcon);
				    ...
				    thumbImageBorder.addEventListener('click', function(e) {
    					 ...
    					 var videoGalleryDetailsView = new VideoGalleryDetailsView(dictionary.images[e.source.imageId].path);
						var videoGalleryDetailsWindow = new VideoGalleryDetailsWindow();
						videoGalleryDetailsWindow.title = 'Video';
						
						Ti.App.SetTitleView(VideoGalleryDetailsWindow,VideoGalleryDetailsView, 'Media' , 0, 1);
						
						videoGalleryDetailsWindow.add(videoGalleryDetailsView);
						
						Ti.App.Tab1.open(videoGalleryDetailsWindow);
    
				    	// thumbGalleryWindow.fireEvent('MediaViewItemSelected',{
				    		// id : e.source.imageId,
				    		// module : 'VideoGalleryDetails',
				    		// videoUrl: Ti.App.ClassPath + 'temp_gallery/testVideo.mp4'
				    	// });
				    });
				}
				else //Image gallery
				{
					thumbImageBorder.addEventListener('click', function(e) {
						Ti.API.info('IMage clicked . clicked id :' + e.source.imageId);
					...
					galleryWindow.add(gallery_view);
					
					Ti.App.SetTitleView(galleryWindow,gallery_view, 'Media' , 0, 1);
					...
					if (isAndroidDevice()) {
						...
						if ( typeof dictionary.windowGroup == 'undefined') {
							...
							Ti.App.Tab1.open(galleryWindow);
						} else {
			...
			if (dictionary.scrollableGallery.displayArrows) {
				...
				if (isAndroidDevice) {
					buttonLeft = Titanium.UI.createButton({
						image : Ti.App.ResourcePath + 'imagegallery/left_arrow.png',
						backgroundImage : Ti.App.ResourcePath + 'imagegallery/invisible_hack.png',
						left : 10,
					...
					buttonRight = Titanium.UI.createButton({
						image : Ti.App.ResourcePath + 'imagegallery/right_arrow.png',
						backgroundImage : Ti.App.ResourcePath + 'imagegallery/invisible_hack.png',
						right : 10,
						width : buttonSize.width * dpi,
						height : buttonSize.height * dpi,
