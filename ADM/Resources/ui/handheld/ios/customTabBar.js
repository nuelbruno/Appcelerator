CustomTabBar = function(settings) {
	...
	var assignClick = function(tabItem) {
		tabItem.addEventListener('click', function(e) {
			...
			if (tabCurrent == pos && settings.currentTab == null) {
				// TODO
				//alert('here');
			...
			var FAQcloseShowService = function(e) {
			  
			   settings.tabBar.tabs[0].active = true;
			   tabCurrent = 0;
			   resetTabs();
			   tabBarItems[0].image = settings.imagePath + settings.items[0].selected;	
			};
	        Ti.App.addEventListener('FAQselectService', FAQcloseShowService);		
	...
	if(settings.currentTab != null || settings.currentTab != undefined)
	{
	  var selectIndex = settings.currentTab;
	  tabBarItems[settings.currentTab].image = settings.imagePath + settings.items[selectIndex].selected;	
	}
	else{
		tabBarItems[0].image = settings.imagePath + settings.items[0].selected;
	}
